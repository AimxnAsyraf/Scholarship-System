        ...
        if hybrid_model is not None and baseline_model is not None:
            ...
            try:
                ...
                test_df = pd.DataFrame([student_data] * num_scholarships)
                    ...
                    # Reset index and add scholarship information
                    unique_scholarships = unique_scholarships.reset_index(drop=True)
                    test_df = test_df.reset_index(drop=True)
                
                    # Add actual scholarship information using proper index alignment
                    test_df['Applied Scholarship'] = unique_scholarships['Scholarship Name'].reset_index(drop=True).values
                    test_df['Scholarship Level'] = unique_scholarships['Scholarship Level'].reset_index(drop=True).values
                    ...
                    test_merged = pd.merge(
                        ...
                        suffixes=('', '_right')
                                ...
                                # Drop duplicate scholarship columns if they exist
                                cols_to_drop = [col for col in test_merged.columns if col.endswith('_right')]
                                if cols_to_drop:
                ...
                # Predict with both models
                                ...
                                print(f"🔍 test_processed columns: {list(test_processed.columns)}")
                
                                # Ensure test_processed is ready for prediction
